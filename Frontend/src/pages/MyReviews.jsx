...
import { 
  Search, Code, Clock, Trash2, ChevronDown, ChevronUp, 
  AlertTriangle, ShieldAlert, Zap, CheckCircle, Loader2, FileCode, History 
} from 'lucide-react';
      ...
      <div className="flex-1 p-6 md:p-8 overflow-x-hidden">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
            <div>
              <h1 className="text-3xl font-extrabold text-gray-900 dark:text-white mb-2 tracking-tight transition-colors duration-300">Review History</h1>
              <p className="text-gray-500 dark:text-gray-400 transition-colors duration-300">Browse, search, and revisit your past AI code analyses.</p>
            </div>
          ...
          {loading ? (
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-800 p-12 text-center shadow-sm transition-colors duration-300">
              ...
              <p className="text-gray-500 dark:text-gray-400 text-sm">
                {searchTerm ? "No snippets matched your search." : "You haven't analyzed any code yet."}
              </p>
            </div>
          ) : (
            <div className="space-y-4">
              {filteredReviews.map((review) => {
                ...
                return (
                  <div 
                    key={review.id} 
                    className={`bg-white dark:bg-slate-900 border transition-all duration-300 shadow-sm overflow-hidden
                      ${isExpanded ? 'border-indigo-200 dark:border-indigo-500/30 rounded-2xl' : 'border-gray-100 dark:border-slate-800 rounded-xl hover:border-indigo-100 dark:hover:border-indigo-500/20 hover:shadow-md'}
                    `}
                  >
                    {/* Compact Header Row */}
                    <div 
                    ...
                    >
                      <div className="flex items-center gap-4 flex-1 min-w-0">
                        ...
                        <div className="min-w-0">
                          <h4 className="font-semibold text-gray-900 dark:text-white truncate">
                            {review.code ? review.code.split('\n')[0].trim() || "Unnamed Snippet" : "Empty Snippet"}
                          </h4>
                          <div className="flex items-center gap-3 text-xs text-gray-500 dark:text-gray-400 mt-1">
                            <span className="flex items-center gap-1"><Clock size={12} /> {formatDate(review.timestamp)}</span>
                            <span>•</span>
                            <span>{review.linesOfCode || 0} lines</span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-3 shrink-0">
                        <div className="flex gap-2">
                          {counts.total === 0 ? (
                            <span className="px-2.5 py-1 rounded-md bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 text-xs font-semibold flex items-center gap-1 border border-emerald-100 dark:border-emerald-500/20">
                        ...
                        <div className="w-px h-6 bg-gray-200 dark:bg-slate-700 hidden sm:block mx-1"></div>
                      ...
                      <div className="border-t border-gray-100 dark:border-slate-800 bg-gray-50/50 dark:bg-slate-900/50 p-6 flex flex-col lg:flex-row gap-6">
                        ...
                        <div className="flex-1 min-w-0">
                          <h5 className="text-sm font-bold text-gray-700 dark:text-gray-300 mb-3 flex items-center gap-2">
                            <FileCode size={16}/> Original Snippet
                          </h5>
                        ...
                        <div className="flex-1 min-w-0">
                          <h5 className="text-sm font-bold text-gray-700 dark:text-gray-300 mb-3 flex items-center gap-2">
                            <Zap size={16}/> Analysis Feedback
                          </h5>
                          
                          <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2 no-scrollbar">
                            {counts.total === 0 ? (
                               <div className="bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-100 dark:border-emerald-500/20 p-4 rounded-xl flex items-center gap-3">
                                ...
                                {review.reviewContent?.bugs?.map((bug, i) => (
                                  <div key={`bug-${i}`} className="bg-white dark:bg-slate-800 border border-rose-100 dark:border-rose-500/20 p-4 rounded-xl shadow-sm">
                                    <div className="flex items-center gap-2 mb-2">
                                      ...
                                      <span className="font-bold text-sm text-gray-900 dark:text-white">{bug.title}</span>
                                    </div>
                                ...
                                {review.reviewContent?.security?.map((sec, i) => (
                                  <div key={`sec-${i}`} className="bg-white dark:bg-slate-800 border border-purple-100 dark:border-purple-500/20 p-4 rounded-xl shadow-sm">
                                    <div className="flex items-center gap-2 mb-2">
                                      ...
                                      <span className="font-bold text-sm text-gray-900 dark:text-white">{sec.title}</span>
                                    </div>
                                ...
                                {review.reviewContent?.improvements?.map((imp, i) => (
                                  <div key={`imp-${i}`} className="bg-white dark:bg-slate-800 border border-amber-100 dark:border-amber-500/20 p-4 rounded-xl shadow-sm">
                                    <div className="flex items-center gap-2 mb-2">
                                      ...
                                      <span className="font-bold text-sm text-gray-900 dark:text-white">{imp.title}</span>
                                    </div>
